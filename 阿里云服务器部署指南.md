...
pm2 save
pm2 startup
# 执行上面命令输出的命令（通常是 sudo env PATH=... pm2 startup systemd -u root --hp /root）
```
...
server {
    ...
    # 如果使用IP访问，可以注释掉server_name，或使用 _ 表示所有域名
    # server_name _;
...
编辑 `app/src/main/java/com/example/yjcy/utils/DomesticRedeemCodeManager.kt`：
...
### 7.2 在AndroidManifest.xml中添加网络权限
...
在 `MainActivity.kt` 中，将 `FirebaseRedeemCodeManager` 替换为 `DomesticRedeemCodeManager`。
...
### Q: 无法连接数据库？
A: 检查MySQL是否运行：`systemctl status mysql`
   检查用户权限：`mysql -u redeem_api -p`

### Q: API返回500错误？
A: 查看PM2日志：`pm2 logs redeem-api`
   检查数据库连接配置

### Q: Nginx 502错误？
A: 检查Node.js服务是否运行：`pm2 list`
   检查端口3000是否监听：`netstat -tlnp | grep 3000`
...
现在你的API应该可以正常工作了。在Android客户端中配置好BASE_URL后，就可以开始使用了！
